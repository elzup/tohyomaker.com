				...
				<ul>
					...
					if (!empty($surveys))
					{
						foreach ($surveys as $survey)
						{
							...
							?>
							<li class="row">
								<span class="title col-sm-7 col-xs-12"><a <?= attr_href(PATH_VOTE, $survey->id) ?>><?= $survey->title ?></a><span class="visible-inline-xs">(<?= $survey->total_num ?>)</span></span>
								<span class="total-num col-sm-2 hidden-xs"><?= $voted_tag ?><?= $survey->total_num ?></span>
								<span class="remain col-sm-3 hidden-xs"><?= ($survey->state == SURVEY_STATE_END) ? '' : tag_icon(ICON_TIME, TRUE) . $survey->get_time_remain_str() ?></span>
							</li>
							<?php
						}
						?>
